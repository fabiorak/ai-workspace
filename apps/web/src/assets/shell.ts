...
${HOME_SIDEBAR}
    ...
    <section aria-labelledby="general-heading" id="general-inbox">
      ...
      <p data-i18n="generalIntro">Questions are immutable evidence you wrote yourself, not yet verified, and CONFIDENTIAL by default. Restricted high-confidence values are blocked before persistence. Search is literal: it does not find paraphrases, typos, synonyms, or stems.</p>
      ...
      <form id="general-link-form" hidden>
        ...
        <p class="effect" data-i18n="generalLinkEffect">Effect: creates a link and nothing else. The General question and the project remain separate and byte-unchanged; no ownership, active memory, Work Item, permission, model, or execution is created.</p>
    ...
    <section aria-labelledby="search-heading" id="search">
      ...
      <form id="search-form">
        ...
        <details id="search-refine">
          ...
          <select id="search-type" name="type"><option value="">All event types</option><option value="USER_MESSAGE">User message</option><option value="AGENT_MESSAGE">Assistant message</option><option value="TOOL_CALL">Tool call</option><option value="TOOL_RESULT">Tool result</option><option value="COMMAND_RESULT">Command result</option><option value="FILE_CHANGE">File change</option><option value="TEST_RESULT">Test result</option><option value="ERROR">Error</option><option value="UNKNOWN">Unrecognized</option></select>
    ...
    <section aria-labelledby="memory-heading" id="memory" hidden>
      ...
      <p>Active memory is a deliberate local statement linked to canonical evidence. <strong>Curating an item does not make it trusted, verified, or true.</strong></p>
      ...
      <form id="memory-add-form">
        ...
        <select id="memory-type" required><option value="DECISION">Decision</option><option value="CONSTRAINT">Constraint</option><option value="FAILURE">Known failure</option></select>
        ...
        <p id="memory-add-effect" class="effect">Effect: creates an item that is active, not yet verified, and not yet assessed for privacy. Evidence remains UNTRUSTED and nothing is executed.</p>
      </form>
      <form id="memory-filter-form">
        <label for="memory-validity">Items to show</label>
        <select id="memory-validity"><option value="">Active only (safe default)</option><option value="SUPERSEDED">Replaced by a newer item</option><option value="INVALIDATED">Withdrawn</option></select>
    ...
    <section aria-labelledby="instructions-heading" id="instructions" hidden>
      ...
      <p class="notice" data-i18n="instructionWarning">Read-only preview: nothing is persisted or executed. Configuring something here does not grant it permission to run.</p>
    ...
    <section aria-labelledby="context-heading" id="context-pack" hidden>
      ...
      <p class="notice" data-i18n="privacyPreflightWarning">Required review boundary: every included item is classified for one explicit model policy. Unknown items default to CONFIDENTIAL. Every valid decision is recorded in the separate local non-content audit before its report is returned. A reviewable outcome is neither permission nor delivery, and detection is not complete PII coverage.</p>
      ...
      <p class="notice" data-i18n="customerAliasWarning">Exact, case-sensitive customer and project aliases only. The dictionary is transient and never persisted. Every result starts as a suggestion you have not reviewed and requires individual confirmation; this is not identity proof, complete PII detection, transformation, or delivery.</p>
