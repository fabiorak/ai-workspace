...
<body>
  ...
  <main id="main" tabindex="-1">
    ...
    <section aria-labelledby="context-heading" id="context-pack" hidden>
      ...
      <form id="pseudonymization-form"><label for="pseudonym-mapping-id">New mapping-set identity / Nuova identità mapping set</label><input id="pseudonym-mapping-id" required maxlength="256" autocomplete="off"><label for="pseudonym-selections">Reviewed selection JSON array / Array JSON delle selezioni revisionate</label><p class="help">Each entry / Ogni voce: {"itemId":"…","contentSha256":"64 lowercase hex","byteStart":0,"byteEnd":4,"entityType":"CUSTOMER"}. Types: PERSON, CUSTOMER, EMAIL, BUSINESS_IDENTIFIER, OTHER.</p><textarea id="pseudonym-selections" required spellcheck="false"></textarea><label for="pseudonym-key">Volatile 32-byte key as 64 lowercase hexadecimal characters / Chiave volatile di 32 byte come 64 caratteri esadecimali minuscoli</label><input id="pseudonym-key" type="password" required pattern="[a-f0-9]{64}" minlength="64" maxlength="64" autocomplete="new-password" spellcheck="false"><button type="submit">Transform, encrypt mapping, and verify locally / Trasforma, cifra il mapping e verifica localmente</button><p class="effect">Effect / Effetto: creates one immutable AES-256-GCM ciphertext document. The key is submitted only to the local loopback process and is cleared from this form after the attempt; key custody and recovery remain your responsibility.</p></form>
...
(() => {
  ...
  document.getElementById("pseudonymization-form").addEventListener("submit", async (event) => { event.preventDefault(); const error = document.getElementById("pseudonymization-error"); const content = document.getElementById("pseudonymization-content"); const keyField = document.getElementById("pseudonym-key"); text(error, ""); if (!selectedProject || !selectedWork || !selectedHandoff) { text(error, "Inspect one immutable handoff first / Esamina prima un handoff immutabile."); return; } try { const profilePath = document.getElementById("privacy-profile-path").value.trim(); const profileDigest = document.getElementById("privacy-profile-digest").value.trim(); const policyPath = document.getElementById("privacy-policy-path").value.trim(); const policyDigest = document.getElementById("privacy-policy-digest").value.trim(); const paths = document.getElementById("privacy-bundles").value.split(/\\r?\\n/u).map((value) => value.trim()).filter(Boolean); const model = document.getElementById("privacy-model").value.trim(); const task = document.getElementById("privacy-task").value.trim(); const selections = JSON.parse(document.getElementById("pseudonym-selections").value); const review = { schemaVersion: 1, mappingSetId: document.getElementById("pseudonym-mapping-id").value.trim(), projectId: selectedProject, workItemId: selectedWork, handoffId: selectedHandoff, modelId: model, attribution: "USER_REVIEWED", selections }; const value = await api(workPath() + "/" + encodeURIComponent(selectedWork) + "/handoffs/" + encodeURIComponent(selectedHandoff) + "/pseudonymization/preview", { method: "POST", body: JSON.stringify({ profile: { path: profilePath, ...(profileDigest ? { expectedDigest: profileDigest } : {}) }, policy: { path: policyPath, ...(policyDigest ? { expectedDigest: policyDigest } : {}) }, bundles: paths.map((path) => ({ path })), model, ...(task ? { task } : {}), review, mappingKeyHex: keyField.value }) }); const counts = value.transformation.accounting; text(document.getElementById("pseudonymization-status"), "Verified local round trip / Round trip locale verificato: " + counts.reviewedSelections + " reviewed selection(s), " + counts.transformedItems + " transformed item(s), mapping " + value.mapping.mappingSetId + " stored as authenticated ciphertext. Not authorized or delivered / Non autorizzato né inviato."); text(content, JSON.stringify(value, null, 2)); content.hidden = false; content.focus(); } catch (cause) { content.hidden = true; text(error, cause.message); document.getElementById("pseudonym-selections").focus(); } finally { keyField.value = ""; } });
