...
<body>
  ...
  <main id="main" tabindex="-1">
    ...
    <section aria-labelledby="work-heading" id="work-items" hidden>
      <h2 id="work-heading" tabindex="-1">Work Items</h2>
      <p>A Work Item is explicit USER_CURATED objective state. No current task or agent is inferred.</p>
      <form id="work-create-form"><label for="work-objective">Software objective</label><textarea id="work-objective" required></textarea><button type="submit">Create proposed Work Item</button><p class="effect">Effect: creates PROPOSED state linked to the currently selected canonical event.</p></form>
    ...
    <section aria-labelledby="work-detail-heading" id="work-detail" hidden>
      <h2 id="work-detail-heading" tabindex="-1">Work Item lifecycle</h2><dl id="work-metadata"></dl><p id="work-objective-detail"></p><ul id="work-transitions"></ul>
    ...
    <section aria-labelledby="handoff-builder-heading" id="handoff-builder" hidden>
      <h2 id="handoff-builder-heading" tabindex="-1">Build a transparent handoff</h2>
    ...
    <section aria-labelledby="handoff-detail-heading" id="handoff-detail" hidden>
      <h2 id="handoff-detail-heading" tabindex="-1">Immutable handoff</h2><pre id="handoff-content" tabindex="0"></pre><button id="handoff-validate" type="button">Validate current Git state</button><div id="handoff-validation" role="status" aria-live="polite"></div><button id="handoff-successor" type="button">Prepare successor</button><button id="handoff-back" type="button">Return to handoff builder</button><p id="handoff-detail-error" class="error" role="alert"></p>
    </section>
    ...
    <section aria-labelledby="capabilities-heading">
      ...
      <ul><li><strong>Available now:</strong> Projects, safe sample import, history search, source inspection, active memory, Work Items, and immutable handoffs.</li><li><strong>GUI parity next:</strong> Effective instruction preview.</li><li><strong>Not active:</strong> Agents, models, tools, external network, handoff evaluation, and instruction execution.</li></ul>
    </section>
  </main>
...
(() => {
  ...
  const renderProjects = (projects) => {
    ...
    for (const project of projects) {
      ...
      select.addEventListener("click", () => { selectedProject = project.id; sessionStorage.setItem("aiw-project", project.id); text(guidance, "Project selected. Continue to evidence, memory, or continuity work below."); importSection.hidden = false; searchSection.hidden = false; memorySection.hidden = false; workSection.hidden = false; text(importStatus, "Ready to import the fictional sample into " + project.name + "."); loadMemory(); loadWork(); nextStep.focus(); });
  ...
  const showHandoff = async (id) => { try { const value = await api(workPath() + "/" + encodeURIComponent(selectedWork) + "/handoffs/" + encodeURIComponent(id)); selectedHandoff = value.id; text(document.getElementById("handoff-content"), JSON.stringify(value, null, 2)); handoffDetail.hidden = false; document.getElementById("handoff-detail-heading").focus(); } catch (cause) { text(document.getElementById("handoff-error"), cause.message); } };
  ...
  if (selectedProject) { importSection.hidden = false; searchSection.hidden = false; memorySection.hidden = false; workSection.hidden = false; text(importStatus, "A project is selected. You can import or re-import the safe sample."); loadMemory(); loadWork(); }
  loadProjects();
})();
`;
