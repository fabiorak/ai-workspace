...
| Sprint 42 | E0, E2, E1-E7 | Screen restricted data per record and chart the state   |
...
[Sprint 42](sprints/SPRINT-042.md) completed what the first real dogfooding
attempt exposed. Sprint 41 could import real transcripts in principle, but every
session that had worked on this repository quoted its own synthetic
`-----BEGIN PRIVATE KEY-----` canary, and file-level fail-closed screening
refused those imports whole; the sessions that did the work were the ones that
could not be read. Screening is now per record inside the tolerant adapter: a
record carrying a high-confidence pattern produces no event, never reaches an
artifact, a reason, or a log, and is counted as `RESTRICTED_DATA:<category>`
from a closed detector set. The provider-neutral core keeps failing closed on
whatever it receives, and user acknowledgement, looser detectors, and allowlists
stay rejected.
[ADR-0030](../adr/0030-screen-restricted-data-per-record-in-the-tolerant-reader.md)
records the decision and amends ADR-0029 on that point only. The same sprint
turned the Sprint 39 dashboard from four counters into charted local state:
accessible inline SVG generated by tested pure functions, served as a
presentation fragment by the local server, with a textual and tabular equivalent
for every figure and no chart dependency, remote asset, or telemetry. Each chart
that counts a subset links to the destination page already filtered to exactly
that subset, so a number can be opened rather than only read.
